...
@group(0) @binding(2) var<storage, read> K: array<f16>;       // [seqLen, numKVHeads, headDim]
@group(0) @binding(3) var<storage, read> V: array<f16>;       // [seqLen, numKVHeads, headDim]
...
fn main(
    ...
    for (var kvBlock: u32 = 0u; kvBlock < numKVBlocks; kvBlock = kvBlock + 1u) {
        ...
        if (kLoadIdx < seqLen) {
            ...
            for (var d: u32 = 0u; d < headDim; d = d + 1u) {
                shared_K[threadIdx * headDim + d] = f32(K[k_offset + d]);
            }
        } else {
            for (var d: u32 = 0u; d < headDim; d = d + 1u) {
        ...
        if (vLoadIdx < seqLen) {
            ...
            for (var d: u32 = 0u; d < headDim; d = d + 1u) {
                shared_V[threadIdx * headDim + d] = f32(V[v_offset + d]);
            }
        } else {
            for (var d: u32 = 0u; d < headDim; d = d + 1u) {
