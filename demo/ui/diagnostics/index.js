...
function formatDiagnosticsMetricsLine(payload, fallbackSuite = null) {
  ...
  if (suite === 'inference' || suite === 'debug') {
    ...
    if (Number.isFinite(metrics.tokensGenerated)) {
      ...
      const ttft = Number.isFinite(metrics.firstTokenMs) ? `${metrics.firstTokenMs}ms` : '--';
      const decode = Number.isFinite(metrics.decodeTokensPerSec)
        ? `${metrics.decodeTokensPerSec} tok/s`
        : '--';
      const overall = Number.isFinite(metrics.tokensPerSec) ? `${metrics.tokensPerSec} tok/s` : '--';
      return `Tokens ${metrics.tokensGenerated} • ${overall} • Prefill ${prefill} • TTFT ${ttft} • Decode ${decode}`;
    }
  }
  return '';
}
...
function formatDiagnosticsOutputSummary(payload, fallbackSuite, captureOutput) {
  ...
  if (suite === 'inference' || suite === 'debug') {
    ...
    if (typeof output === 'string' && output.trim().length > 0) {
      const cleaned = output.replace(/<[^>]{1,32}>/g, '').replace(/\s+/g, ' ').trim();
      const display = cleaned.length > 0 ? cleaned : output.replace(/\s+/g, ' ').trim();
      const preview = display.length > 240 ? `${display.slice(0, 240)}...` : display;
