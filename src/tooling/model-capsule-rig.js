...
import { runRigPipeline } from '../converter/forge-stages.js';
...
export function usage() {
  return [
    'Doppler Rig: deterministic signed Capsule v2 compiler',
    '',
    'Usage:',
    '  node tools/rig-model-capsule.js --program-bundle <path> --release-manifest <path> --out <capsule.json>',
    '  node tools/rig-model-capsule.js --manifest <path> --reference-report <path> --release-manifest <path> --out <capsule.json> [--conversion-config <path>]',
    '  node tools/rig-model-capsule.js --config <path|json>',
    '',
    'Flags:',
...
export async function buildRigOptions(
  flags,
...
async function loadQualificationEvidence(reportPaths, bundle) {
  if (bundle.referenceTranscript?.operation === 'embed') {
    ...
    for (const reportPath of reportPaths || []) {
      ...
      if (transcript.operation !== 'embed' || transcript.modelId !== bundle.modelId
        ...
        throw new Error('Rig embedding qualification must use the same model, source reference, and request.');
      }
  ...
  if (bundle.referenceTranscript?.operation === 'rerank') {
    ...
    for (const reportPath of reportPaths || []) {
      ...
      if (transcript.operation !== 'rerank' || transcript.modelId !== bundle.modelId
        ...
        throw new Error('Rig rerank qualification must use the same model, source reference, and request.');
      }
      const observed = await hashFile(reportPath);
  ...
  if (bundle.referenceTranscript?.operation === 'encodeSequence') {
    ...
    for (const reportPath of reportPaths || []) {
      ...
      if (transcript.operation !== 'encodeSequence' || transcript.modelId !== bundle.modelId
        ...
        throw new Error('Rig sequence qualification must use the same model, reference, and request.');
      }
      const observed = await hashFile(reportPath);
      evidence.push({
  ...
  if (!Array.isArray(referenceTokens) || referenceTokens.length === 0) {
    throw new Error('Rig qualification requires Program Bundle reference token IDs.');
  }
  const evidence = [];
  for (const reportPath of reportPaths || []) {
    ...
    if (report.modelId !== bundle.modelId) {
      throw new Error(`Rig qualification report modelId "${report.modelId}" does not match "${bundle.modelId}".`);
    }
    if (!report.results?.some((result) => result?.name === 'generation' && result?.passed === true)) {
      throw new Error(`Rig qualification report "${reportPath}" has no passed generation result.`);
    }
    if (typeof surface !== 'string' || !surface.endsWith('-webgpu')) {
      throw new Error(`Rig qualification report "${reportPath}" lacks an explicit WebGPU surface.`);
    }
    if (!generationConfig || !Number.isFinite(generationConfig.temperature)) {
      throw new Error(`Rig qualification report "${reportPath}" lacks explicit generationConfig.`);
    }
    if (generationConfig.temperature > 0 && !Number.isFinite(generationConfig.seed)) {
      throw new Error(`Rig qualification report "${reportPath}" is stochastic without an explicit seed.`);
    }
    if (!Array.isArray(tokens)
      || tokens.length !== referenceTokens.length
      || tokens.some((tokenId, index) => tokenId !== referenceTokens[index])) {
      throw new Error(`Rig qualification report "${reportPath}" does not exactly match the Program Bundle transcript.`);
    }
    if (transcript.executionGraphHash !== bundle.execution.graphHash) {
      throw new Error(`Rig qualification report "${reportPath}" execution graph does not match the Program Bundle.`);
    }
...
async function verifySourceArtifactBytes(bundle, bundlePath, repoRoot) {
  ...
  for (const artifact of bundle.artifacts) {
    ...
    if (observed.hash !== artifact.hash || observed.sizeBytes !== artifact.sizeBytes) {
      throw new Error(
        `Rig source artifact mismatch for "${artifact.path}": ` +
...
async function materializeProgramBundle(options) {
  ...
  if (!options.outputPath) throw new Error('Doppler Rig requires outputPath.');
  ...
  if (!createdAtUtc) {
    throw new Error('Rig requires a stable --created-at or a timestamp in the reference report.');
  }
...
async function resolveSigner(options) {
  ...
  if (!privateKeyPath || !publicKeyPath || !authority) {
    throw new Error(
      'Doppler Rig requires explicit signingPrivateKeyPath, signingPublicKeyPath, and signingAuthority.'
    );
  }
...
async function materializeCapsuleArtifactClosure(capsule, sourceBundle, sourceBundlePath, repoRoot, outputPath, qualificationEvidence = [], modelIREvidence = null) {
  ...
  for (const artifact of capsule.artifacts) {
    ...
    if (artifact.role === 'program-bundle') {
      ...
      if (!match) throw new Error(`Rig cannot materialize Capsule artifact "${artifact.artifactId}".`);
      match.used = true;
    ...
    if (relative.startsWith('..') || path.isAbsolute(relative)) {
      throw new Error(`Rig Capsule artifact path escapes output root: ${artifact.path}.`);
    }
    ...
    if (observed.hash !== artifact.hash || observed.sizeBytes !== artifact.sizeBytes) {
      throw new Error(`Rig materialized artifact verification failed for "${artifact.path}".`);
    }
  }
}

export async function rigModelCapsule(options) {
  if (!options?.outputPath) throw new Error('Doppler Rig requires --out / outputPath.');
  if (!options?.releaseManifestPath) {
    throw new Error('Doppler Rig requires --release-manifest / releaseManifestPath.');
  }
  ...
  const { capsule, stages, searchReceipt } = await runRigPipeline({
