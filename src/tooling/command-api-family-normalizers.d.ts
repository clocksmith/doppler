export declare function normalizeConvert(raw: Record<string, unknown>): Record<string, unknown>;
export declare function normalizeRefreshIntegrity(raw: Record<string, unknown>): Record<string, unknown>;
export declare function normalizeRelease(raw: Record<string, unknown>): Record<string, unknown>;
export declare function normalizeTrainingOperatorCommand(
  raw: Record<string, unknown>,
  command: string
): Record<string, unknown>;
export declare function normalizeSuiteCommand(
  raw: Record<string, unknown>,
  command: string
): Record<string, unknown>;
