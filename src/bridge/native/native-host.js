...
export async function handleListRequest(reqId, payload) {
...
export async function handleReadRequest(reqId, payload) {
    ...
    while ((chunk = process.stdin.read()) !== null) {
      ...
      while (buffer.length >= 4) {
        ...
        try {
          ...
          log.error('DopplerBridge', `Error handling message: ${err.message}`);
        }
      }
    }
  });
...
main().catch(err => {
  log.error('DopplerBridge', `Fatal error: ${err.message}`);
  process.exit(1);
});
