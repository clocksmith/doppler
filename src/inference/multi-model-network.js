...
export class MultiModelNetwork {
   ...
   *
   * @param {string} expertId - The expert to use for all temporal states (typically 'base')
   * @param {{ description: string; maxTokens?: number; convergenceThreshold?: number }} task - Task with description, maxTokens, and optional convergenceThreshold
   * @param {{ turns?: number; temperatureStart?: number; temperatureDecay?: number; temperatureMin?: number; shortcutInterval?: number; enableShortcuts?: boolean }} [config={}] - Configuration for turns, temperature decay, and shortcuts
   * @returns {Promise<{ finalOutput: string; history: Array<{ turn: number; output: string; timestamp: number; role: 'seed' | 'reflect' | 'refine' }>; turnsUsed: number; converged: boolean }>} Final output, full history, and turns used
   */
  async executeTemporalRing(expertId, task, config = {}) {
    const {
      turns = 5,
    ...
    /** @type {Array<{ turn: number; output: string; timestamp: number; role: 'seed' | 'reflect' | 'refine' }>} */
    const history = [];
    let currentOutput = '';
    let converged = false;
   ...
   * @param {number} turn
   * @param {Array<{ turn: number; output: string; role: string }>} history
   * @param {string} lastOutput
   * @param {'seed' | 'reflect' | 'refine'} role
   * @returns {string}
   * @private
   */
  buildTemporalPrompt(taskDescription, turn, history, lastOutput, role) {
    if (role === 'seed') {
   ...
   * @param {Array<{ output: string }>} history
   * @param {number} [threshold]
   * @returns {boolean}
   * @private
   */
  detectTemporalConvergence(currentOutput, history, threshold) {
