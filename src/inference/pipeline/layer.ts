...
import { createTensor, type TensorDtype } from '../../gpu/tensor.js';
import {
  doAttention, doRMSNorm, doResidualAdd, doMatmul,
  releaseOrTrack
} from './ops.js';
import {
...
import type { AttentionConfig, AttentionState } from './attention.js';
import type { LayerContext, SandwichNormInfo } from './types.js';

// Track if we've logged one-time messages (avoid spam)
// let loggedFusedDownNorm = false; // Moved to ffn.ts
...
export async function processLayerGPU(
  ...
  if (context.debug) {
    ...
    // Debug attention output for configured layers (default: 0, 2, 17 where Q4K issues were found)
  ...
  let postAttn: import('../../gpu/tensor.js').Tensor;
  if (residualFused) {
    postAttn = attnOutput;
  ...
  let outputTensor: import('../../gpu/tensor.js').Tensor;
    ...
    for (const step of steps) {
      switch (step.op) {
        ...
        case 'ffn': {
          ...
          let outputTensor: import('../../gpu/tensor.js').Tensor;
          // Dynamically import to avoid cycle if necessary, but we are using ops/ffn so it should be fine.
          // We moved FFN logic to ffn.ts so we must use it.
          // But wait, ffn.ts has runDenseFFNGPU etc.
