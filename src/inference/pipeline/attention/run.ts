...
import { type WeightBuffer, type CpuWeightBuffer, isWeightBuffer, getWeightDtype } from '../../../gpu/weight-buffer.js';
...
export async function runLayerAttentionGPU(
  ...
  if (hasCache) {
    ...
    if (isKernelDebugEnabled(layerIdx)) {
      ...
      await dumpKVCache(state.kvCache as unknown as KVCache, layerIdx);
    }
  } else {
