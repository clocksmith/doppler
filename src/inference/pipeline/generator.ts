...
import { computeLogits, computeLogitsGPU, recordLogitsGPU, extractLastPositionLogits, applySoftcapping, type LogitsConfig, type LogitsWeights } from './logits.js';
import { createWeightBufferHelpers, type WeightBufferConfig } from './weights.js';
...
import type { RouterWeights } from './types.js';
...
export class PipelineGenerator {
  ...
  private _buildLayerContext(recorder?: CommandRecorder, isDecodeMode: boolean = false, debugLayers?: number[] | null): LayerContext {
    const config = this.state.modelConfig!;
    ...
    return {
      ...
      expertLoader: this.state.dopplerLoader as ExpertLoader | null,
      moeRouter: this.state.moeRouter,
      layerRouterWeights: this.state.layerRouterWeights as Map<number, { weight: GPUBuffer | Float32Array; bias: GPUBuffer | Float32Array | null }> | undefined,
      recorder,
