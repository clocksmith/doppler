import { PACK_ADAPTER_POLICY, resolvePackAdapterSet } from '../../config/pack-adapters.js';
import { hashPackObservation, normalizePackObservation } from '../../config/pack-operation.js';
...
export function createPackAdapterExecution({ program, pack, targetPlan }) {
  return async function prepare(request, { adapterArtifactStore, signal }) {
    const entries = resolvePackAdapterSet(request.adapterSet === undefined ? PACK_ADAPTER_POLICY.legacyAdapterSet : request.adapterSet,
      { pack, targetPlan, operation: request.operation.name });
    ...
    try {
      for (const entry of entries) {
        ...
        signal.throwIfAborted();
        const runtimeIdentity = freezePackV2(normalizePackObservation(program.getActiveAdapterIdentity()));
        ...
        receipts.push(freezePackV2({ identity: entry.identity, requestDigest: hashPackObservation(entry),
