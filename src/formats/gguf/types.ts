...
export function parseGGUFHeader(buffer: ArrayBuffer): GGUFParseResult {
...
export function identifyMoETensors(parsed: GGUFParseResult): Map<number, Map<number, GGUFTensor[]>> {
  const result = new Map<number, Map<number, GGUFTensor[]>>();
  const layers = groupTensorsByLayer(parsed);

  for (const [layerIdx, tensors] of layers.entries()) {
    const experts = new Map<number, GGUFTensor[]>();
    for (const tensor of tensors) {
