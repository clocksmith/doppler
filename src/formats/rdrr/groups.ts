...
export function getGroup(groupId: string): ComponentGroup | null {
...
export function getShardsForGroup(groupId: string): number[] {
  return getManifest()?.groups?.[groupId]?.shards ?? [];
}

export function getTensorsForGroup(groupId: string): string[] {
...
export function getLayerGroupIds(): string[] {
  const ids = Object.keys(getManifest()?.groups || {})
    .filter(id => id.startsWith('layer.'));
  return sortGroupIds(ids);
}

export function getExpertGroupIds(layerIdx: number): string[] {
