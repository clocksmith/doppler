...
export class RDRRWriter {
  ...
  private manifest = {
    ...
    groups: {} as Record<string, ComponentGroupSchema>,
    tensorsFile: TENSORS_FILENAME,
    tensorCount: 0,
    moeConfig: null as MoEConfigSchema | null,
    totalSize: 0,
   ...
   * Uses classifyTensor() to determine group membership.
   */
  ...
  async writeTokenizer(tokenizer: TokenizerConfig): Promise<void> {
    if (!tokenizer.tokens || tokenizer.tokens.length === 0) {
      log.warn('RDRRWriter', 'No vocab tokens found, skipping tokenizer bundling');
    ...
    log.verbose('RDRRWriter', `Wrote tokenizer.json (${tokenizer.tokens.length} tokens, type: ${type})`);
  ...
  async writeHuggingFaceTokenizer(tokenizerJson: HuggingFaceTokenizer): Promise<void> {
    if (!tokenizerJson || !tokenizerJson.model) {
      log.warn('RDRRWriter', 'Invalid HuggingFace tokenizer.json, skipping');
      return;
    }
    ...
    log.verbose('RDRRWriter', `Wrote HuggingFace tokenizer.json (${vocabSize} tokens, type: ${type})`);
  ...
  async finalize(): Promise<WriteResult> {
    ...
    // Build external tensors.json (v1 format)
    ...
    for (const groupId of sortedGroupIds) {
      const tensors = this.groupTensorMap.get(groupId) || [];
      const shards = Array.from(this.groupShardMap.get(groupId) || new Set<number>()).sort((a, b) => a - b);
      const dataChunks = this.groupDataMap.get(groupId) || [];
    ...
    if (this.manifest.moeConfig && this.expertShardMap.size > 0) {
      ...
      if (this.sharedExpertIndices.size > 0) {
        ...
        log.verbose('RDRRWriter', `Shared experts: ${this.manifest.moeConfig.sharedExperts.join(', ')}`);
      }

      log.verbose('RDRRWriter', `MoE expert mapping: ${this.expertShardMap.size} experts, ~${(expertBytes / 1024 / 1024).toFixed(1)}MB each`);
    }

    const manifestPath = join(this.outputDir, 'manifest.json');
    await writeFile(manifestPath, JSON.stringify(this.manifest, null, 2));

    log.verbose('RDRRWriter', `Wrote ${Object.keys(groups).length} component groups`);
...
export async function writeRDRR(
  ...
  try {
    ...
    if (config?.expertCount || config?.num_local_experts) {
      ...
      log.verbose('RDRRWriter', `MoE config: ${numExperts} experts, ${numExpertsPerToken} active per token`);
    }
...
// Re-export test model helper (extracted to separate module)
