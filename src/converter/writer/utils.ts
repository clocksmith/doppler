 ...
 */
export async function computeHash(data: Uint8Array, algorithm: HashAlgorithm = 'sha256'): Promise<string> {
  if (algorithm === 'blake3') {
    try {
 ...
 */
export function alignOffset(offset: number, alignment = ALIGNMENT): number {
 ...
 */
export function getBytesPerElement(dtype: string): number {
 ...
 */
export function transpose2D(data: Uint8Array, rows: number, cols: number, dtype: string): Uint8Array {
