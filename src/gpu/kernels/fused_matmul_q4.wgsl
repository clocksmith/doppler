...
// eliminating redundant global reads across columns.
// Use when K <= SHARED_A_MAX (compile-time override).
// Total shared memory: SHARED_A_MAX*4 + 256*4 = e.g. 2048*4+1024 = 9216 bytes.
// ============================================================================
override SHARED_A_MAX: u32 = 3584u;

var<workgroup> shared_A_buf: array<f32, SHARED_A_MAX>;
var<workgroup> multicol_sums_sa: array<f32, MAX_WORKGROUP_SIZE>;

@compute @workgroup_size(WORKGROUP_SIZE, 1, 1)
fn main_multicol_shared(
    @builtin(local_invocation_id) lid: vec3<u32>,
    @builtin(workgroup_id) wg_id: vec3<u32>,
    @builtin(subgroup_invocation_id) sg_id: u32,
    @builtin(subgroup_size) sg_size: u32
        ...
        for (var b: u32 = tid_in_col; b < full_blocks; b = b + THREADS_PER_COL_GEMV) {
            ...
            for (var sb: u32 = 0u; sb < NUM_SUBBLOCKS; sb = sb + 1u) {
                ...
                for (var i: u32 = 0u; i < SUBBLOCK_SIZE; i = i + 4u) {
                    ...
                    let a0 = shared_A_buf[k0];
                    let a1 = shared_A_buf[k0 + 1u];
                    let a2 = shared_A_buf[k0 + 2u];
                    let a3 = shared_A_buf[k0 + 3u];
            ...
            if (tail_block % THREADS_PER_COL_GEMV == tid_in_col) {
                ...
                for (var sb: u32 = 0u; sb < NUM_SUBBLOCKS; sb = sb + 1u) {
                    ...
                    for (var i: u32 = 0u; i < SUBBLOCK_SIZE; i = i + 4u) {
                        ...
                        var a2: f32 = 0.0;
                        var a3: f32 = 0.0;
                        if (k0 < u.K) { a0 = shared_A_buf[k0]; }
                        if (k0 + 1u < u.K) { a1 = shared_A_buf[k0 + 1u]; }
                        if (k0 + 2u < u.K) { a2 = shared_A_buf[k0 + 2u]; }
                        if (k0 + 3u < u.K) { a3 = shared_A_buf[k0 + 3u]; }
    ...
    // Reduction within each column's 8 threads
    multicol_sums_sa[local_id] = partial_sum;
    workgroupBarrier();
    ...
    if (tid_in_col == 0u && is_valid) {
        ...
        let base = col_in_wg * THREADS_PER_COL_GEMV;
        for (var i: u32 = 0u; i < THREADS_PER_COL_GEMV; i = i + 1u) {
            final_sum = final_sum + multicol_sums_sa[base + i];
        }
        C[col] = final_sum * u.alpha;
    }
}
