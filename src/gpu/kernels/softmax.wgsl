...
// =============================================================================
// SUBGROUP-ACCELERATED VARIANTS
// =============================================================================
// Use subgroupMax/subgroupAdd for 3-5x faster reductions vs tree reduction.
...
// Subgroup-accelerated softmax - one workgroup per row
// Uses subgroupMax/subgroupAdd to reduce barriers from 16 → 2
@compute @workgroup_size(WORKGROUP_SIZE, 1, 1)
fn main_subgroup(
    @builtin(local_invocation_id) local_id: vec3<u32>,
    @builtin(workgroup_id) wg_id: vec3<u32>,
    ...
    // Final max reduction - thread 0 finds global max
    if (thread_idx == 0u) {
