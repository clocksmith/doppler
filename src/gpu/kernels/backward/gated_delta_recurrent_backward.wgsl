...
@group(0) @binding(4) var<storage, read> log_decay: array<f32>;
@group(0) @binding(5) var<storage, read> beta: array<f32>;
@group(0) @binding(6) var<storage, read> state_history: array<f32>;
@group(0) @binding(7) var<storage, read> grad_output: array<f32>;
@group(0) @binding(8) var<storage, read_write> grad_query: array<f32>;
@group(0) @binding(9) var<storage, read_write> grad_key: array<f32>;
@group(0) @binding(10) var<storage, read_write> grad_value: array<f32>;
@group(0) @binding(11) var<storage, read_write> grad_log_decay: array<f32>;
@group(0) @binding(12) var<storage, read_write> grad_beta: array<f32>;
@group(0) @binding(13) var<storage, read_write> grad_state: array<f32>;
...
fn main(
    ...
    for (var reverse_token: u32 = 0u; reverse_token < u.num_tokens; reverse_token = reverse_token + 1u) {
        ...
        let decay = exp(log_decay[scalar_index]);
        var memory: f32 = 0.0;
        if (active) {
            for (var key_index: u32 = 0u; key_index < u.key_dim; key_index = key_index + 1u) {
                ...
                memory = memory
                    + previous_state
                    * decay
                    * key[vector_index(token, head, key_index, u.key_dim)];
            }
        }
        let value_offset = vector_index(token, head, lane, u.value_dim);
        let value_at_token = select(0.0, value[value_offset], active);
        let delta = (value_at_token - memory) * beta[scalar_index];
        let output_gradient = select(0.0, grad_output[value_offset], active);
        ...
        for (var key_index: u32 = 0u; key_index < u.key_dim; key_index = key_index + 1u) {
            ...
            if (lane == 0u) {
                grad_query[query_offset] = shared_reduce[0];
            }
            if (active) {
        ...
        if (active) {
            ...
            grad_value[value_offset] = grad_delta * beta[scalar_index];
        }
        let grad_memory = -grad_delta * beta[scalar_index];
        shared_reduce[lane] = select(0.0, grad_delta * (value_at_token - memory), active);
        workgroupBarrier();
        for (var stride: u32 = WORKGROUP_SIZE / 2u; stride > 0u; stride = stride / 2u) {
            if (lane < stride) {
                shared_reduce[lane] = shared_reduce[lane] + shared_reduce[lane + stride];
        ...
        if (lane == 0u) {
            grad_beta[scalar_index] = shared_reduce[0];
        }
        workgroupBarrier();
        ...
        for (var key_index: u32 = 0u; key_index < u.key_dim; key_index = key_index + 1u) {
            ...
            let gradient_before_decay_update = select(0.0, grad_state[gradient_state_offset], active);
            ...
            if (lane == 0u) {
                grad_key[key_offset] = shared_reduce[0];
            }

            if (active) {
                let gradient_decayed_state = gradient_before_decay_update
                    + grad_memory * key[key_offset];
        ...
        shared_reduce[lane] = select(0.0, local_grad_decay, active);
        workgroupBarrier();
        for (var stride: u32 = WORKGROUP_SIZE / 2u; stride > 0u; stride = stride / 2u) {
            if (lane < stride) {
                shared_reduce[lane] = shared_reduce[lane] + shared_reduce[lane + stride];
        ...
        if (lane == 0u) {
            grad_log_decay[scalar_index] = shared_reduce[0];
        }
        workgroupBarrier();
    }
}
