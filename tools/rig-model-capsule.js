...
import {
  RIG_VERSION,
  buildRigOptions as buildRigOptionsCore,
  rigModelCapsule,
  parseArgs,
  readJsonInput,
  usage,
} from '../src/tooling/model-capsule-rig.js';
import {
...
export async function buildRigOptions(flags, metaUrl = import.meta.url) {
  const options = await buildRigOptionsCore(flags, metaUrl);
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const receipt = await rigModelCapsule(await buildRigOptions(flags));
  if (flags.json) {
    console.log(JSON.stringify(receipt, null, 2));
    return;
  }
  console.log('✔ Doppler Rig: signed Capsule v2 compiled');
