...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (!['--spec', '--config', '--out'].includes(token)) throw new Error(`Unknown argument "${token}".`);
  ...
  if (!options.out || Boolean(options.spec) === Boolean(options.config)) {
    throw new Error('Usage: --spec <source-spec> --out <receipt.json> OR --config <onboarding.json> --out <directory>');
  }
  return options;
}
...
if (options.config) {
  ...
  const result = await runModelOnboarding(config, { sourceRoot: repoRoot, outputDir: path.resolve(options.out) });
