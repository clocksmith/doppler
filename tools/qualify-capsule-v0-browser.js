...
const DEFAULT_OUT = 'reports/capsule-v0/gemma-3-270m-it-q4k-ehf16-af32/browser-qualification.json';

function parseArgs(argv) {
  const options = { capsule: DEFAULT_CAPSULE, reference: DEFAULT_REFERENCE, out: DEFAULT_OUT, channel: 'chrome', maxTokens: null };
  for (let index = 0; index < argv.length; index += 1) {
    const token = argv[index];
    if (!['--capsule', '--reference', '--out', '--channel', '--max-tokens'].includes(token)) throw new Error(`Unknown argument "${token}".`);
...
export async function qualifyCapsuleV0Browser(options = parseArgs(process.argv.slice(2))) {
  ...
  try {
    ...
    const url = new URL('/tests/capsule/browser-capsule-qualification.html', server.baseUrl).href;
    await page.goto(url, { waitUntil: 'load' });
    const result = await page.evaluate(async ({ capsulePath, referencePath, maxTokens }) => {
      ...
      const [{ openCapsule }, { CAPSULE_V0_TRUSTED_SIGNERS }] = await Promise.all([
        import('/src/index-browser.js'),
        import('/src/config/capsule-v0-trusted-signers.js'),
      ]);
      ...
      const session = await openCapsule(new URL(`/${capsulePath}`, location.href).href, {
        trustedSigners: CAPSULE_V0_TRUSTED_SIGNERS,
      });
      ...
      try {
        ...
        return {
          schema: 'doppler.capsule-v0-browser-qualification/v1',
    ...
    }, { capsulePath: options.capsule, referencePath: options.reference, maxTokens: options.maxTokens });
    ...
    if (!result.passed) throw new Error(`Browser Capsule parity failed at token ${result.firstMismatch}.`);
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  qualifyCapsuleV0Browser().catch((error) => {
