...
import { buildCapsuleV3, signCapsuleV3, getCapsuleIdentity, signCapsuleReleaseEvent } from '../src/capsule.js';
...
if (args['init-signer']) {
  ...
  if (signerPath.startsWith(root + path.sep)) throw new Error('Private signer cannot be inside the distributable Capsule directory.');
  ...
  for (const entry of artifacts) {
    ...
    if (!file.startsWith(root + path.sep)) throw new Error('Artifact path escapes Capsule');
  ...
  const capsule = await signCapsuleV3(buildCapsuleV3({ ...executable, artifacts, targetPlans: [plan] }), signer);
  const identity = getCapsuleIdentity(capsule);
  const source = capsule.modelIR.sourceIdentity;
  ...
  const release = { schema: 'doppler.capsule-release/v1', source: {
    ...
    provenanceDigest: computeCanonicalSha256(capsule.modelIR.provenance),
    ...
    stateSnapshot: { schema: 'doppler.capsule-state-snapshot/v1', format: applicationContract.state.format,
  ...
  const event = await signCapsuleReleaseEvent({ capsule: { schema, semanticRoot, envelopeDigest }, sequence: 1,
  ...
  await fs.writeFile(path.join(root, 'capsule.json'), JSON.stringify(capsule));
  ...
  console.log(JSON.stringify({ capsule: identity, artifactBytes: artifacts.reduce((sum, a) => sum + a.sizeBytes, 0), expiresAtUtc }));
}
