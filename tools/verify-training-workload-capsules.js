...
function parseArgs(argv) {
  const parsed = {
    root: 'src/experimental/training/workload-capsules',
    registry: 'src/experimental/training/workload-capsules/registry.json',
    out: null,
    writeRegistry: false,
  };
...
async function collectWorkloadEntries(rootPath, registryPath) {
  ...
  for (const filePath of files) {
    const loaded = await loadTrainingWorkloadCapsule(filePath);
  ...
  if (workloads.length === 0) {
    throw new Error(`No workload capsules found in ${rootPath}.`);
  }
  return workloads;
}
