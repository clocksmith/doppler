...
import { openCapsule } from '../src/index.js';
import { destroyDevice } from '../src/gpu/device.js';
import { releaseNodeWebGPU } from '../src/tooling/node-webgpu.js';
import { CAPSULE_V0_TRUSTED_SIGNERS } from '../src/config/capsule-v0-trusted-signers.js';

const REPO_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const DEFAULT_CAPSULE = 'reports/capsule-v0/gemma-3-270m-it-q4k-ehf16-af32/model.capsule.json';
const DEFAULT_REFERENCE = 'reports/gemma-3-270m-it-q4k-ehf16-af32/2026-08-22T18-14-30.244Z.json';
const DEFAULT_OUT = 'reports/capsule-v0/gemma-3-270m-it-q4k-ehf16-af32/node-qualification.json';

function parseArgs(argv) {
  const options = { capsule: DEFAULT_CAPSULE, reference: DEFAULT_REFERENCE, out: DEFAULT_OUT };
  for (let index = 0; index < argv.length; index += 1) {
    const token = argv[index];
    if (!['--capsule', '--reference', '--out'].includes(token)) throw new Error(`Unknown argument "${token}".`);
...
export function createCapsuleNodeQualificationReceipt({
  capsule,
  session,
  generatedTokenIds,
  expectedTokenIds,
  ...
  if (!declaredIdentity?.digest || !observedIdentity?.digest) {
    throw new Error('Node Capsule qualification requires declared and observed initial execution identities.');
  }
  if (declaredIdentity.digest !== observedIdentity.digest) {
    throw new Error(
      `Node Capsule initial execution identity mismatch: declared ${declaredIdentity.digest}, ` +
      `observed ${observedIdentity.digest}.`
    );
  }
  const digestAfter = session.selectedTargetPlanDigest;
  return {
    schema: 'doppler.capsule-node-qualification/v2',
    ...
    signature: {
      authority: capsule.signature?.authority ?? null,
      algorithm: capsule.signature?.algorithm ?? null,
      publicKeyDigest: capsule.signature?.publicKeyDigest ?? null,
      disposition: 'explicitly-trusted-for-qualification',
    },
    closure: {
      artifacts: capsule.artifacts.length,
      verifiedArtifacts: session.verification?.artifactReceipts?.length ?? 0,
      wgslModules: capsule.wgslModules.length,
      qualifiedEntryPoints: [...(capsule.modelIR?.supportScope?.qualifiedEntryPoints ?? [])],
    },
...
export async function withNodeCapsuleQualificationLifecycle(openSession, runSession, lifecycle = {}) {
...
export async function qualifyCapsuleV0Node(options = parseArgs(process.argv.slice(2))) {
  const capsulePath = path.resolve(REPO_ROOT, options.capsule);
  ...
  return withNodeCapsuleQualificationLifecycle(
    () => openCapsule(capsulePath, { trustedSigners: CAPSULE_V0_TRUSTED_SIGNERS }),
    async (session) => {
      ...
      const receipt = createCapsuleNodeQualificationReceipt({
        capsule,
        session,
      ...
      if (!receipt.passed) throw new Error(`Node Capsule parity failed at token ${receipt.firstMismatch}.`);
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  qualifyCapsuleV0Node().catch((error) => {
