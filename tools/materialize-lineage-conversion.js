...
const receipt = materializeLineageConversionCandidate({
...
if (options.check) {
  ...
  if (observedOutput !== outputText || observedReceipt !== receiptText) {
    throw new Error('Lineage conversion outputs are stale. Run without --check.');
  }
} else {
  await Promise.all([
...
console.log(`${recipe.modelId}: ${receipt.conversionConfigDigest}`);
