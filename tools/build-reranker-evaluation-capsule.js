...
export function assertRerankerObservedShaderClosure(report, modules) {
  if (!Array.isArray(report.requests)) throw new Error('Evaluation Capsule requires observed shader requests.');
  ...
  if (!modelShaders.length) throw new Error('Evaluation Capsule requires observed model shader requests.');
  const declared = new Set(modules.map(module => module.file));
  const missing = modelShaders.filter(file => !declared.has(file));
  if (missing.length) throw new Error(`Evaluation Capsule closure omits observed model shaders: ${missing.join(', ')}.`);
...
export async function buildRerankerEvaluationCapsule(config) {
  return buildOperationEvaluationCapsule(config, 'rerank');
}

export async function buildEmbeddingEvaluationCapsule(config) {
  if (typeof config?.applicationId !== 'string' || !config.applicationId.trim()) throw new Error('Embedding Capsule requires an explicit applicationId.');
  return buildOperationEvaluationCapsule(config, 'embed');
}

async function buildOperationEvaluationCapsule(config, operation) {
  for (const field of ['qualificationPath', 'conversionConfigPath', 'licensePath', 'applicationPath', 'outputDir', 'authorityId']) {
    if (typeof config?.[field] !== 'string' || !config[field].trim()) throw new Error(`Evaluation Capsule requires ${field}.`);
  }
  if (!Number.isSafeInteger(config.revocation?.offlineExpirySeconds)
    ...
    throw new Error('Evaluation Capsule requires an explicit fail-closed revocation policy.');
  }
  ...
  if (report.schema !== expectedSchema || !report.passed
    ...
    throw new Error(`Evaluation Capsule requires passing ${operation} source qualification.`);
  }
  if (!['browser-webgpu', 'node-webgpu'].includes(report.runtime?.surface)) {
    throw new Error('Evaluation Capsule requires an explicitly observed WebGPU surface.');
  }
  ...
  const forge = { repoRoot: ROOT, manifestPath, modelDir: path.dirname(manifestPath),
    ...
    initialExecutionIdentityPath: path.resolve(config.qualificationPath), outputPath: path.join(outputDir, 'distribution/capsule.json'),
  ...
  if (hashBytesSha256(await fs.readFile(licensePath)) !== capsule.release.source.license.textDigest) {
  ...
  await write('build-receipt.json', { result, capsuleIdentity: getCapsuleIdentity(capsule), application, observedShaderClosure,
    modelLicense: { path: licensePath, digest: capsule.release.source.license.textDigest,
      binding: 'release.source.license.textDigest', artifactInventoryMember: false },
    evidenceClass: 'internal-source-qualified-evaluation-capsule', physicalCapsuleExecution: false, externalAdoption: false });
  return result;
}
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    if (config.operation !== undefined && !['embed', 'rerank'].includes(config.operation)) throw new Error('Unsupported evaluation Capsule operation.');
    console.log(JSON.stringify(await (config.operation === 'embed' ? buildEmbeddingEvaluationCapsule(config) : buildRerankerEvaluationCapsule(config))));
  } catch (error) {
