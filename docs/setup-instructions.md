...
For a canonical source→convert→verify→bench workflow (adaptable to any model), use:
...
1. **First token (TTFT):** Time to first token (model-load + first emitted token)
2. **Decode Speed (tok/s):** Token generation rate
3. **First response:** First-token latency plus model load
4. **Total Time:** End-to-end generation time
...
=== DOPPLER PERFORMANCE METRICS ===
{
  ...
  "metrics": {
    ...
    "prefillMs": 220,
    "decodeMs": 830,
    "prefillTokensPerSec": 210.4,
    "decodeTokensPerSec": 106.4
  }
}
```
...
- [operations.md](operations.md) - Troubleshooting and runtime operations
