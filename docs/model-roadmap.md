...
| [Meta Muse Glimmer 30B](programs/glimmer-architectural-generalization.md) | source-truth-only | Text, vision, and projector topology are pinned. A generic lowerability audit now identifies missing local-attention, QK-scale, output-multiplier, postnorm, and no-RoPE contracts; entry points remain unlowered and no execution support is claimed. |
