...
| [Meta Muse Glimmer 30B](programs/glimmer-architectural-generalization.md) | source-truth-only | Text, vision, and projector topology are pinned. A generic audit identifies missing local-attention, QK-scale, output-multiplier, postnorm, and no-RoPE contracts; four operational meanings remain unresolved pending pinned implementation evidence, so no execution support is claimed. |
