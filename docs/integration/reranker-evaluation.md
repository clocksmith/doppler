...
retained under `raw.diagnostic`; they are not a performance sample or Capsule proof.
Failed diagnostic comparisons retain their raw observations. Capture configuration
is rejected in Capsule mode rather than bypassing the signed execution interface.
...
`node tools/build-reranker-evaluation-capsule.js <build-config.json>` takes
...
observed initial execution identity, signed Capsule, application contract, explicit
...
never adds kernels to a sealed Capsule. This covers the observed execution path,
...
and tokenizer identity are additionally bound by conversion and the Capsule's
artifact closure.
...
`--check`, then reconvert and requalify it. Do not synchronize a retained Capsule
...
as an executable artifact in the Capsule inventory.
...
qualification config with `mode: "capsule"`. Supply `capsulePath`, `application`,
`packageBundlePath` identifying that retained bundle,
`authorizedCapsule: { capsuleId, semanticRoot }`, and `openOptions` containing explicit
`trustedSigners` and `acceptedTargetPlanDigests`. These values come from the
local application's reviewed build contract, not an arbitrary downloaded Capsule.

Capsule mode invokes the actual Electron renderer adapter and integrated host,
...
Keep Capsule reference paths origin-relative (for example `/capsule/capsule.json`) so
...
For Capsule v3, retain the complete signed `releaseEvents`, independently selected
...
a weight shard declared in this Capsule. Corruption changes only the served copy;
retained source bytes remain intact. `device-loss` destroys the evaluator's
WebGPU device after Capsule opening. Fault runs remain failed qualification
...
conversion recipes, references, Capsules, and qualifications together. Do not
...
- Capsule execution establishes the observed physical host and signed closure.
...
runtime rerunning the same F16 Capsule on the physical Electron host. Its separate
...
physical run of the F16 Capsule. Rejected candidates and failed diagnostic setups
...
Qwen topology; only reranking is lowered and qualified by its Capsule build.
See the separate `reranker-source-capsule-20260905.json` checkpoint under
...
Capsule qualification config, point `modelDir` at the retained Capsule's
