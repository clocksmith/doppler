...
- `src/config/presets/kernel-paths/<id>.json`
...
1. Copy the nearest existing kernel-path preset.
2. Update `activationDtype`, `kvDtype`, and the relevant `decode`, `prefill`, `preLayer`, `postLayer`, or `sampling` steps.
3. Add the new ID to `src/config/presets/kernel-paths/registry.json`.
4. Wire the new ID into a model preset or conversion config so it is reachable.
...
- Adding the preset file but not the registry entry.
- Registering the preset but never wiring it into a model preset or conversion config.
...
- `src/config/presets/kernel-paths/registry.json`
- `src/config/presets/kernel-paths/gemma3-q4k-dequant-f32a-online.json`
- [../config.md](../config.md)
