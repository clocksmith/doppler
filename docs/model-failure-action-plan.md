...
Last updated: 2026-03-12T18:18:00Z
Plan status: active
...
| --- | --- | --- | --- | --- |
| `H-GEMMA-Q4K` | Gemma 3 Q4K failures are in the Q4K artifact/runtime path, not generic Gemma 3 execution | `in_progress` | Conversion triage now passes on refreshed local artifacts: source checkpoints are BF16, sampled local shard hashes match, sampled `q_proj` Q4K blocks match the source-derived quantization bytes exactly, and exact runtime smokes diverge by model (`1B` coherent, `270M` incoherent) under the same execution-v0 path | Continue runtime-path audit with focus on model-specific numeric divergence rather than generic conversion corruption |
