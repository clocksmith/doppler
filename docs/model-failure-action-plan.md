...
| `H-QWEN-LA` | Qwen 3.5 failure is driven by linear-attention correctness or Qwen-specific runtime semantics | `done` | Root cause identified and fixed: `detectRmsNormWeightOffset` in `manifest-inference.js` hardcoded `rmsNormWeightOffset: true` for Qwen 3.5, overriding the correct preset value of `false`. This applied Gemma-style `(1+w)*x` RMS norm instead of standard `w*x`, causing ~2x scaling at every norm layer. Fix: removed the `isQwen35LinearAttentionConfig` guard; corrected all three Qwen manifests in place. Awaits real Class `A` runtime re-smoke to confirm coherent output | Re-smoke with corrected manifests |
