...
- Qwen 3.6 27B f16 sibling, using the existing Qwen 3.6 Q4K weight capsule
...
| Reference-run `weightsRef` preflight | Done | `run-program-bundle-reference.js` checks shared weight-capsule files for manifest-only siblings instead of requiring local shard copies. |
...
- Qwen af16 must share the existing Qwen weight capsule through `weightsRef`; it must not duplicate shards.
...
The af16 sibling pins the legacy target `artifactIdentity.weightCapsuleHash` as the
