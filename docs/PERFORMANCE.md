...
## Benchmark Workflow (2026 Baseline)

This section defines the default benchmark workflow and baseline targets for 2026-ready measurements.
...
- `gpt-oss-20b` (once model assets are ready)
...
To reduce cold-start variance, enable kernel prewarm or auto-tuning before benchmark runs via runtime config. When `tooling.intent = "calibrate"`, the runtime defaults `kernelWarmup.prewarm` to `true` unless explicitly set.
...
The demo ships a manifest file at `/demo/bench-manifest.json`. You can run it directly from DevTools:
...
Update this document with the measured TTFT and TPS values.
