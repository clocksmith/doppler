...
immutable `adapterSet` in the request. Each `doppler.pack-adapter/v1` entry binds
...
See the [adapter declarations](../../src/config/pack-adapters.d.ts) for the full
...
Module IDs must belong to the signed initial execution closure and include the
registered operations required by [adapter policy](../../src/config/pack-adapters.json).
