...
### Stage 1 — candidate graph

1. Build the candidate as an execution-v1 graph or explicit execution graph
   transform. Do not add string kernel-path IDs or a side registry.
2. Test the candidate through a runtime config that carries an inline
   execution-v1-derived `runtime.inference.kernelPath` object, or through the
   conversion config before artifact promotion.
...
4. Perf gate: run `bench` with the candidate on the target hardware. The
