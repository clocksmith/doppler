...
- Emit JSON that conforms to `../../benchmarks/benchmark-schema.json`.
...
| JSON result schema | ✓ Implemented | `../../benchmarks/benchmark-schema.json` |
...
### Canonical Timing Contract (Required for Cross-Engine Compare)

- All DOPPLER vs vendor comparisons must normalize timing fields to:
  ...
  - `timing.totalRunMs`
  - `timing.modelLoadMs`
  - `timing.decodeMsPerTokenP50`
  - `timing.decodeMsPerTokenP95`
  - `timing.decodeMsPerTokenP99`
- `timing.firstResponseMs` is explicitly `timing.modelLoadMs + timing.firstTokenMs` (unless implementation cannot separate pre-load and prefill phases, then document exception).
- `timing.cacheMode` and `timing.loadMode` are required; accepted values are `warm|cold` and `opfs|http|memory`.

### Latency and Throughput

- `firstTokenMs`: time from generation start to first token emitted.
- `firstResponseMs`: model-load + first token latency.
- `prefillMs`: wall time for prompt prefill.
- `decodeMs`: wall time for decode loop (generated tokens only).
- `totalRunMs`: model load + prefill + decode, end-to-end wall time.
- `decodeMsPerTokenP50`, `decodeMsPerTokenP95`, `decodeMsPerTokenP99`: per-token decode distributions.
- `prefillTokensPerSec`: `promptTokens / (prefillMs / 1000)`.
- `decodeTokensPerSec`: `generatedTokens / (decodeMs / 1000)`.
...
- Cache/run mode must be explicitly reported in output as `timing.cacheMode` and `timing.loadMode`; missing fields are invalid for compare runs.
...
```json
{
  ...
  "metrics": {
    "firstTokenMs": 820,
    "firstResponseMs": 1420,
    "prefillMs": 760,
    "decodeMs": 3120,
    "totalRunMs": 3940,
    "decodeMsPerTokenP50": 35.4,
    "decodeTokensPerSec": 41,
    "prefillTokensPerSec": 505,
...
## Vendor Comparison Policy

## Vendor Benchmarking Tooling (Required)

Competitive benchmarking MUST run through the registry-driven toolchain:

- Targets and metadata in `benchmarks/vendors/registry.json`
- Shared workloads in `benchmarks/vendors/workloads.json`
- Capability matrix in `benchmarks/vendors/capabilities.json`
- Target normalization in `benchmarks/vendors/harnesses/*.json`
- Runner implementation in `benchmarks/runners/transformersjs-bench.js` (extend similarly for additional targets)
- Orchestration in:
  - `tools/compare-engines.js` for side-by-side Doppler vs vendor runs
  - `tools/vendor-bench.js` for registry/run/import/validation
- Normalized comparison artifacts in `benchmarks/vendors/results/` using `benchmarks/vendors/schema/result.schema.json`
...
```json
{
  "comparison": {
    "vendor": "WebLLM",
    "vendorVersion": "0.2.80",
    "vendorResult": {
      "model": "Llama-3.1-8B-Instruct-q4f16_1-MLC",
      "decodeTokensPerSec": 41.1,
