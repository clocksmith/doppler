...
The primary bug was in `src/converter/quantizer.js`. Our quantizer produced data in a **different format** than what the GPU dequantizer (`gpu/kernels/dequant_shared.wgsl`) expected.
...
Rewrote `quantizeQ4KBlock()` in `src/converter/quantizer.js`:
...
| File | Change |
|------|--------|
| `src/converter/quantizer.js` | Rewrote Q4_K encoding to match llama.cpp format |
...
- `src/converter/quantizer.js`: CPU quantization for model conversion
