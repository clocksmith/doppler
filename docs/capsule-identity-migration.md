# Executable Capsule identity and release events

Capsule v2 remains readable. Its timestamp and release fields remain in its
historical semantic root. Never rewrite or re-sign a historical v2 document in
place. Capsule v3 contains only the executable ModelIR, TargetPlans, WGSL modules,
...
The new source API is exported as `doppler-gpu/capsule`. It is not a claim that a
published package, customer release, or peer network already consumes v3.

```js
import { migrateCapsuleV2, getCapsuleIdentity, signCapsuleReleaseEvent } from 'doppler-gpu/capsule';

const migrated = await migrateCapsuleV2(oldCapsule, { trustedSigners, signer });
const { schema, semanticRoot, envelopeDigest } = getCapsuleIdentity(migrated.capsule);
const event = await signCapsuleReleaseEvent({
  capsule: { schema, semanticRoot, envelopeDigest },
  sequence: 1,
...
`openCapsule` facade with the built-in program adapter. Both use the same verifier.
...
Consumers calling `verifyCapsule()` directly must likewise retain the checkpoint
on `CapsuleReleaseStateError`; verification alone does not own storage.
...
already persisted release checkpoint and exact Capsule bytes:
...
Capsule envelope, application contract, artifact closure, qualified TargetPlan,
...
time, whether managed eligibility expired, and the retained decision. Capsule v3
...
Capsule opening filters by device capability, host qualification, optional
...
eligible plans only and never grants authorization. Signed Capsule order breaks
...
For example, `openCapsule(capsule, { requiredOperations: ['rerank'],
...
`encodeSequence` returns a `doppler.capsule-execution-receipt/v1`
bound to the exact Capsule, selected TargetPlan, full artifact receipts, release
...
separate. Run `npm run capsule-v2:check` for v2/v3 regression coverage and
`node tools/run-node-tests.js tests/capsule/capsule-retained-local-use.test.js` for
