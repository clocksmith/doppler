...
class PipelineBenchmark {
  ...
  async runInference(prompt, isWarmup) {
    ...
    const generator = this.pipeline.generate(prompt, {
      ...
      onToken: (id, t) => {
        ...
        if (tokenCount === 1) {
          ...
          const vocabSize = this.manifest?.config?.vocab_size ?? 32e3;
    ...
    if (pipelineStats.gpuTimePrefillMs !== void 0) {
      gpuTimePrefillMs = pipelineStats.gpuTimePrefillMs;
    }
    if (pipelineStats.gpuTimeDecodeMs !== void 0) {
  ...
  aggregateMetrics(runs, loadMetrics) {
    ...
    const metrics = {
      ...
      prefill_tokens_per_sec: avgPrefill > 0 ? Math.round(avgPromptTokens / (avgPrefill / 1e3)) : 0,
      decode_ms_total: Math.round(avgDecodeTotal),
      decode_tokens_per_sec: avgDecodeTotal > 0 ? Math.round((avgGeneratedTokens - 1) / (avgDecodeTotal / 1e3)) : 0,
    ...
    const gpuPrefillTimes = runs.map((r) => r.gpuTimePrefillMs).filter((t) => t !== void 0);
    const gpuDecodeTimes = runs.map((r) => r.gpuTimeDecodeMs).filter((t) => t !== void 0);
    ...
    if (loadMetrics.opfsUsageBytes !== void 0) {
      metrics.opfs_usage_bytes = loadMetrics.opfsUsageBytes;
    }
    if (loadMetrics.storagePersisted !== void 0) {
  ...
  getModelInfo() {
    ...
    return {
      ...
      modelName: m?.name ?? m?.modelName ?? void 0,
      ...
      numLayers: m?.config?.num_hidden_layers ?? m?.numLayers ?? void 0,
      hiddenSize: m?.config?.hidden_size ?? m?.hiddenSize ?? void 0
    };
  }
...
async function runQuickBenchmark(modelPath) {
...
async function runFullBenchmark(modelPath) {
  const results = [];
