...
export const TEST_CAPSULE_AUTHORITY = 'doppler-capsule-test';
export const TEST_CAPSULE_PUBLIC_KEY = Object.freeze({
  crv: 'Ed25519',
...
export function createCapsuleReleaseFixture(options = {}) {
  const targetIds = options.targetIds ?? ['webgpu-f32-portable'];
  const previousCapsuleId = options.previousCapsuleId ?? 'capsule-test-model-capsule-v2-previous';
  const previousSemanticRoot = options.previousSemanticRoot ?? `sha256:${'9'.repeat(64)}`;
  return {
    schema: 'doppler.capsule-release/v1',
    source: {
      repository: 'https://example.invalid/capsule-test-model',
    ...
    application: {
      applicationId: 'capsule-test-application',
    ...
    exclusions: {
      ...
      known: [{
        ...
        reason: 'The reference Capsule is qualified only for its declared target.',
        evidenceDigest: `sha256:${'1'.repeat(64)}`,
      }],
    },
    lifecycle: {
      releaseVersion: '1.0.0',
      supersedes: { capsuleId: previousCapsuleId, semanticRoot: previousSemanticRoot },
      migration: {
    ...
    revocation: {
      authorityId: 'doppler-capsule-test-authority',
...
export async function createSignedCapsuleFixture(options = {}) {
  ...
  const unsignedCapsule = buildCapsuleV2({
    modelId: modelIR.modelId,
    ...
    release: options.release ?? createCapsuleReleaseFixture({ targetIds: (options.targetPlans ?? [targetPlan]).map((plan) => plan.targetId) }),
  });
  const capsule = await signCapsuleV2(unsignedCapsule, {
    authority: TEST_CAPSULE_AUTHORITY,
    privateKeyJwk: TEST_CAPSULE_PRIVATE_KEY,
  ...
  return { capsule, modelIR, targetPlan, artifactBytes, artifactStore };
}
