...
} = await import('../../src/inference/pipelines/structured/json-head-pipeline.js');
...
for (const modelType of DREAM_MODEL_TYPES) {
  ...
  assert.equal(isStructuredJsonHeadModelType(modelType), true, `${modelType} should be supported`);
}
assert.equal(isDreamStructuredModelType('transformer'), false);
assert.equal(isStructuredJsonHeadModelType('transformer'), false);
assert.equal(isDreamStructuredModelType(''), false);
assert.equal(isStructuredJsonHeadModelType(''), false);

{
  const legacy = new DreamStructuredPipeline();
  assert.ok(legacy instanceof StructuredJsonHeadPipeline);
}
