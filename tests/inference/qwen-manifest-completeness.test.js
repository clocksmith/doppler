...
const manifest = await loadJson('../../models/local/qwen-3-5-0-8b-f16/manifest.json');
...
{
  assert.equal(hasExecutionV0(manifest.inference), false);
}
...

{
  ...
  for (let i = 0; i < lp.layerTypes.length; i++) {
    ...
    assert.equal(lp.layerTypes[i], expected, `layer ${i}`);
  }
}
