...
import { migrateCapsuleV2, getCapsuleIdentity, signCapsuleReleaseEvent, verifyCapsule, verifyCapsuleReleaseEvents } from '../../src/capsule.js';
...
const policy = { now: '2026-09-03T00:00:00.000Z', minimumSequence: 1, checkpoint,
  retainedLocalUse: { schema: 'doppler.capsule-retained-local-use/v1', capsule: params.capsule,
...
const verify = (history = [eligible], override = policy) => verifyCapsuleReleaseEvents(history, { capsule, trustedSigners, policy: override });
...
assert.ok(Object.isFrozen(authorized.authorization.retainedLocalUse.capsule));
...
for (const change of [
  ...
  p => { p.retainedLocalUse.capsule.semanticRoot = badDigest; },
  p => { p.retainedLocalUse.capsule.envelopeDigest = badDigest; },
  p => { p.retainedLocalUse.capsule.schema = 'doppler.capsule/v2'; },
  p => { p.retainedLocalUse.capsule.extra = true; },
...
await assert.rejects(verifyCapsule(fixture.capsule, { trustedSigners: { [TEST_CAPSULE_AUTHORITY]: TEST_CAPSULE_PUBLIC_KEY },
  artifactStore: fixture.artifactStore, releasePolicy: policy }), /requires Capsule v3/);
...
await assert.rejects(runtime.openCapsule(capsule, { ...options, persistReleaseCheckpoint: async () => { throw new Error('store unavailable'); } }), /store unavailable/);
...
const request = { schema: 'doppler.capsule-operation-request/v1', operation: { name: 'encodeSequence', version: 1 },
...
await assert.rejects(createRuntime({ readArtifact: async artifact => new Uint8Array(artifact.sizeBytes) }).openCapsule(capsule, options), /hash or size mismatch/);
assert.equal(opens, 1);

for (const action of ['blocked', 'quarantined', 'revoked', 'superseded']) {
  const denied = await signCapsuleReleaseEvent({ ...params, sequence: 2, previousEventDigest: eligible.digest, action }, signer);
  writes = [];
  await assert.rejects(runtime.openCapsule(capsule, { ...options, releaseEvents: [eligible, denied] }), /blocked/);
  assert.deepEqual(writes, [{ sequence: 2, digest: denied.digest }], 'authenticated denial advances durable checkpoint');
  await assert.rejects(runtime.openCapsule(capsule, { ...options, releasePolicy: { ...policy, minimumSequence: 2, checkpoint: writes[0] } }), /rolled back/);
  await assert.rejects(runtime.openCapsule(capsule, { ...options, releaseEvents: [eligible, denied],
...
const rollback = await signCapsuleReleaseEvent({ ...params, sequence: 3, previousEventDigest: revoked.digest, action: 'rollback-authorized' }, signer);
await assert.rejects(verify([eligible, revoked, rollback]), /blocked/);
const promoted = await signCapsuleReleaseEvent({ ...params, sequence: 2, previousEventDigest: eligible.digest, action: 'promoted' }, signer);
...
for (const history of [[{ ...eligible, action: 'blocked' }], [promoted], [eligible, { ...revoked, signature: eligible.signature }]]) {
  await assert.rejects(runtime.openCapsule(capsule, { ...options, releaseEvents: history }));
}
assert.equal(writes.length, 0, 'invalid signatures, digests or noncontiguous history cannot poison durable state');
assert.equal(opens, 1);
console.log('✔ capsule-retained-local-use.test.js passed');
