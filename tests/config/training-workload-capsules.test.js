...
const tempDir = mkdtempSync(path.join(tmpdir(), 'doppler-training-workload-capsules-'));
try {
...
// stale registry detection: independent block using a single known-good workload
{
  ...
  try {
    ...
    const srcWorkload = readFileSync('src/experimental/training/workload-capsules/lora-toy-tiny.json', 'utf8');
...
console.log('training-workload-capsules.test: ok');
