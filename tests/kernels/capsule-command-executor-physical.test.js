...
if (!probe.ready) {
  ...
  console.log(`capsule-command-executor-physical.test.js: skipped (${probe.reason})`);
  process.exit(0);
}
...
const staging = device.createBuffer({
  label: 'doppler-capsule:physical-readback',
  size: 4,
  usage: GPUBufferUsage.MAP_READ | GPUBufferUsage.COPY_DST,
});
const encoder = device.createCommandEncoder({ label: 'doppler-capsule:physical-readback' });
...
console.log('capsule-command-executor-physical.test.js: physical WebGPU dispatch/readback passed');
