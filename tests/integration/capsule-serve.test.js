...
import { createCapsuleServeHandler } from '../../src/cli/serve/capsule-handler.js';
import { writeCapsuleEvent } from '../../src/cli/serve/capsule-http.js';
...
import { normalizeCapsuleServePolicy } from '../../src/config/capsule-serve.js';
import { createSignedCapsuleFixture, TEST_CAPSULE_AUTHORITY, TEST_CAPSULE_PUBLIC_KEY } from '../helpers/capsule-v2-fixture.js';

// Real HTTP -> public signed Capsule session -> existing operation adapters.
// GPU/program components are synthetic, not physical model qualification.
const token = 'test-only-capsule-serving-token';
const policy = { schema: 'doppler.capsule-serve/v1', maxRequestBytes: 10000,
...
function job(operation, application) {
  ...
  return { schema: 'doppler.capsule-operation-request/v1', operation: { name: operation, version: 1 },
...
async function openFixture(operation) {
  const fixture = await createSignedCapsuleFixture({ operation, ...(operation === 'embed' ? { manifest: {
  ...
  const program = {
    ...
    async rerank({ options }) {
      ...
      return { schema: 'doppler_rerank_evidence/v1', inputHash: hashCapsuleObservation('input'),
        outputHash: hashCapsuleObservation('output'), backendIdentityHash: hashCapsuleObservation('backend'), scores: [1, 0], ranking: [0, 1] };
    },
  ...
  const runtime = createDopplerRuntime({
    ...
    async programFactory({ capsule, targetPlan }) {
      ...
      const manifest = JSON.parse(new TextDecoder().decode(await fixture.artifactStore.readArtifact(capsule.artifacts.find(a => a.artifactId === 'manifest'))));
      ...
      const handle = createModelHandle(pipeline, { modelId: capsule.modelId, manifestHash: capsule.artifacts.find(a => a.artifactId === 'manifest').hash });
      return createCapsuleProgramAdapter(handle, capsule, targetPlan);
    },
  });
  return { session: await runtime.openCapsule(fixture.capsule), fixture,
...
async function serve(test, servingPolicy = policy) {
  const handler = createCapsuleServeHandler({ session: test.session, token, policy: servingPolicy });
...
for (const invalid of [null, {}, { ...policy, extra: true }, { ...policy, maxOutputBytes: null },
  ...
  assert.throws(() => normalizeCapsuleServePolicy(invalid));
}
...
for (const operation of ['generate', 'embed', 'rerank', 'encodeSequence']) {
  ...
  try {
    const request = job(operation, test.fixture.capsule.release.application);
    ...
    assert.deepEqual(await description.json(), { modelId: test.session.modelId, capsule: test.session.capsuleIdentity,
...
try {
  const request = job('rerank', test.fixture.capsule.release.application);
  ...
  await rejection({ ...request, input: { ...request.input, application: {} } }, 500, 'CAPSULE_EXECUTION_FAILED');
...
try {
  ...
  const response = cancellationServer.request(job('rerank', cancelled.fixture.capsule.release.application), { signal: controller.signal });
  const rejected = assert.rejects(response, /abort/i);
  await startedPromise;
  const concurrent = await cancellationServer.request(job('rerank', cancelled.fixture.capsule.release.application));
  ...
  assert.equal((await collect(cancelled.session.executeOperation(job('rerank', cancelled.fixture.capsule.release.application)))).at(-1).status, 'completed');
...
for (const abort of [false, true]) {
  ...
  res.write = () => false;
  const controller = new AbortController();
  const write = writeCapsuleEvent(res, 'event\n', controller.signal);
...
console.log('capsule-serve.test: ok');
