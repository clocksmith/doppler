...
import {
  ...
  resolveCapsuleProgramLoadOptions,
...
const service = createDopplerRuntimeService({
  ...
  async resolveCapsuleInput() { throw new Error('must not resolve a Capsule after policy override rejection'); },
});
await assert.rejects(
  service.openCapsule({}, { modelLoadOptions: { runtimeConfig: { inference: {} } } }),
...
assert.deepEqual(
  resolveCapsuleProgramLoadOptions({ initialExecutionIdentity }),
  { runtimeConfig: programLoadPolicy.runtimeConfig }
);
assert.deepEqual(resolveCapsuleProgramLoadOptions({}), {});

console.log('✔ capsule-load-options.test.js passed');
