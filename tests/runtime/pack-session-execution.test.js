...
import { createPackSessionExecution } from '../../src/client/runtime/pack-session-execution.js';

const deferred = () => { let resolve; const promise = new Promise(done => { resolve = done; }); return { promise, resolve }; };
const scope = createPackSessionExecution();
...
const streaming = createPackSessionExecution();
const order = [];
...
const failure = createPackSessionExecution();
...
assert.equal(failure.run(() => 'recovered'), 'recovered');
await failure.close(() => {});
console.log('pack-session-execution: exclusion, cancellation, draining and cleanup failure passed');
