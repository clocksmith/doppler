...
import { executeCapsuleForecast } from '../../src/client/runtime/capsule-forecast.js';
...
const release = createCapsuleReleaseFixture();
release.lifecycle.supersedes = null;
release.lifecycle.migration = null;
release.lifecycle.failedUpgrade.previousCapsuleId = null;
release.lifecycle.failedUpgrade.previousSemanticRoot = null;
assert.equal(validateCapsuleReleaseContract(release).ok, true);
release.lifecycle.failedUpgrade.previousCapsuleId = 'invented';
assert.equal(validateCapsuleReleaseContract(release).ok, false);
...
const result = await executeCapsuleForecast({ ...base, request });
assert.equal(result.receipt.operation, 'forecast');
assert.match(result.receipt.inputHash, /^sha256:/);
await assert.rejects(executeCapsuleForecast({ ...base, request: { ...request, application: { ...application, applicationRevision: '2' } } }), /application identity/);
await assert.rejects(executeCapsuleForecast({ ...base, request: { ...request, assignmentHash: undefined } }), /Invalid/);
await assert.rejects(executeCapsuleForecast({ ...base, request, program: { async forecast() { return { ...result, values: [NaN] }; } } }), /Malformed/);
const cancelled = new AbortController(); cancelled.abort(new Error('cancelled'));
await assert.rejects(executeCapsuleForecast({ ...base, request: { ...request, signal: cancelled.signal } }), /cancelled/);
assert.equal(validateTargetPlan({ phases: { forecast: [] }, qualification: [{ operation: 'generate' }] }).ok, false);
console.log('✔ capsule-forecast.test.js passed');
