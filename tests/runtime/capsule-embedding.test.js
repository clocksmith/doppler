...
import { migrateCapsuleV2, getCapsuleIdentity, signCapsuleReleaseEvent } from '../../src/capsule.js';
import { CAPSULE_OPERATION_REQUEST_SCHEMA, hashCapsuleObservation } from '../../src/config/capsule-operation.js';

// Connected public Capsule session -> actual program adapter -> actual evidence handle.
...
async function openFixture(options = {}) {
  const fixture = await createSignedCapsuleFixture({ operation: 'embed', manifest, ...options });
  let capsule = fixture.capsule;
  let trustedSigners = { [TEST_CAPSULE_AUTHORITY]: TEST_CAPSULE_PUBLIC_KEY };
  let sessionOptions = {};
  let application = fixture.capsule.release.application;
  let releaseEvent = null;
  let persisted = false;
  if (options.capsuleV3) {
    ...
    const migrated = await migrateCapsuleV2(capsule, { trustedSigners, signer });
    capsule = migrated.capsule;
    ...
    releaseEvent = await signCapsuleReleaseEvent({ capsule: { schema, semanticRoot, envelopeDigest },
  ...
  const handle = createModelHandle(pipeline, { modelId: fixture.capsule.modelId,
    manifestHash: fixture.capsule.artifacts.find(artifact => artifact.artifactId === 'manifest').hash });
  const runtime = createDopplerRuntime({
    ...
    async programFactory({ capsule, targetPlan }) {
      if (options.capsuleV3) assert.equal(persisted, true, 'checkpoint persistence precedes program loading');
      const adapter = createCapsuleProgramAdapter(handle, capsule, targetPlan);
  ...
  return { session: await runtime.openCapsule(capsule, sessionOptions), fixture, pipeline, releaseEvent,
...
assert.deepEqual(result.receipt.capsule, test.session.capsuleIdentity);
...
assert.equal(result.receipt.artifactReceipts.length, test.fixture.capsule.artifacts.length);
...
const operationRequest = (application) => ({ schema: CAPSULE_OPERATION_REQUEST_SCHEMA,
...
assert.equal(batchResult.receipt.assignmentHash, hashCapsuleObservation(batch.assignment));
assert.equal(batchResult.receipt.outputHash, hashCapsuleObservation(batchResult.output));
...
for (const mutate of [
  ...
  await assert.rejects(test.session.embed(test.request), /Malformed Capsule|evidence does not match/);
}
...
const v3 = await openFixture({ capsuleV3: true });
await assert.rejects(v3.session.embed({ ...v3.request, application: v3.fixture.capsule.release.application }), /application identity/);
...
assert.equal(released.receipt.capsule.schema, 'doppler.capsule/v3');
...
await assert.rejects(collect(v3.session.executeOperation(operationRequest(v3.fixture.capsule.release.application))), /application identity/);
...
console.log('capsule-embedding.test: passed (synthetic pipeline/device; actual session, adapter and evidence handle)');
