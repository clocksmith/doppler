...
import { createPackAdapterExecution } from '../../src/client/runtime/pack-adapter-execution.js';
...
const pack = { modelId: 'base', semanticRoot: hash('model'), envelopeDigest: hash('envelope'), artifactClosureDigest: hash('closure') };
const targetPlan = { qualification: [{ operation: 'generate' }],
  initialExecutionIdentity: { kernelClosure: [
    { moduleId: 'matmul', file: 'matmul_f16.wgsl', entry: 'main' },
    { moduleId: 'scale', file: 'scale.wgsl', entry: 'main' },
    { moduleId: 'residual', file: 'residual.wgsl', entry: 'main' }
  ] }, adapterExecution: { schema: 'doppler.pack-adapter-execution/v1', maxAdapters: 1,
    combination: 'single', formats: ['peft_safetensors'], operations: ['generate'], kernelModules: ['matmul', 'scale', 'residual'] } };
const entry = { schema: 'doppler.pack-adapter/v1', identity: hash('adapter'), baseModel: pack, format: 'peft_safetensors',
...
const request = { schema: 'doppler.pack-operation-request/v1', operation: { name: 'generate', version: 1 },
...
const make = (plan = targetPlan) => createPackOperationExecutor({ identity: { pack }, assertCurrent: async () => {},
  prepareExecution: createPackAdapterExecution({ program, pack, targetPlan: plan }),
...
assert.equal(events.at(-1).receipt.requestHash, hashPackObservation(request));
...
await assert.rejects(collect(make()({ ...request, adapterSet: [{ ...entry, baseModel: { ...pack, semanticRoot: hash('other') } }] }, control)), /base model/);
...
const cleanupExecutor = createPackOperationExecutor({ identity: { pack }, assertCurrent: async () => {},
...
const finalCleanupFailure = createPackOperationExecutor({ identity: { pack }, assertCurrent: async () => {},
...
console.log('pack-adapter-execution: identity, declared kernels, corruption, base mismatch, composition, cleanup, cancellation passed');
