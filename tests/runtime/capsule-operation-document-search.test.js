import assert from 'node:assert/strict';
import { createCapsuleOperationExecutor } from '../../src/client/runtime/capsule-operation-executor.js';
import { createCapsuleOperationAdapters } from '../../src/client/runtime/capsule-operation-adapters.js';
import { createCapsuleProgramAdapter } from '../../src/client/runtime/capsule-program-adapter.js';
import { computeCanonicalSha256 } from '../../src/formats/canonical-hash.js';

// Preserve the local document regressions against the shared upstream executor.
const executeCapsuleOperation = (context, request, control) => createCapsuleOperationExecutor({
  identity: { capsule: context.identity, targetId: context.targetId, targetPlanDigest: context.targetPlanDigest,
    artifactReceipts: context.artifactReceipts, releaseEventDigest: context.releaseEventDigest },
  assertCurrent: context.assertCurrent,
  adapters: createCapsuleOperationAdapters({ program: context.program,
    embed: ({ text, options }) => context.program.embed(text, options) }),
})(request, control);

const collect = async (iterator) => {
  const events = [];
  for await (const event of iterator) events.push(event);
  return events;
};
const request = () => ({ schema: 'doppler.capsule-operation-request/v1', operation: { name: 'embed', version: 1 },
  input: { application: { applicationId: 'synthetic-document-search' }, texts: ['one', 'two'] }, options: {}, assignment: null,
  limits: { maxInputBytes: 1024, maxOutputBytes: 1024, deadlineAt: Date.now() + 10000 } });
let calls = 0;
let closed = false;
const context = { identity: { capsuleId: 'fixture' }, targetId: 'fixture', targetPlanDigest: 'fixture',
  artifactReceipts: [], runtimeVersion: 'fixture', releaseEventDigest: null,
  assertCurrent: async () => { if (closed) throw new Error('closed'); },
  program: { embed: async (text) => { calls++; return { embedding: new Float32Array([text.length, 1]) }; } } };
const input = request();
const originalHash = computeCanonicalSha256(input);
const iterator = executeCapsuleOperation(context, input);
input.input.texts[0] = 'changed after invocation';
const events = await collect(iterator);
assert.equal(calls, 2);
assert.deepEqual(events.map((event) => event.status), ['partial', 'partial', 'completed']);
assert.deepEqual(events.at(-1).output.embeddings[0].embedding, [3, 1]);
assert.equal(events.at(-1).receipt.requestHash, originalHash);
for (const [index, event] of events.entries()) {
  const { eventDigest, ...payload } = event;
  assert.equal(eventDigest, computeCanonicalSha256(payload));
  assert.equal(event.eventIndex, index);
  assert.equal(event.previousEventDigest, index ? events[index - 1].eventDigest : null);
}
assert(Object.isFrozen(events.at(-1).output.embeddings[0].embedding));
const before = calls;
assert.throws(() => executeCapsuleOperation(context, { ...request(), options: { precision: 'f16' } }), /fields/);
assert.throws(() => executeCapsuleOperation(context, { ...request(), operation: { name: 'unknown', version: 1 } }), /Unsupported Capsule operation/);
assert.throws(() => executeCapsuleOperation(context, { ...request(), input: { texts: [''] } }), /texts/);
const oversized = request(); oversized.limits.maxInputBytes = 1;
assert.throws(() => executeCapsuleOperation(context, oversized), /maxInputBytes/);
assert.equal(calls, before);
const expired = request(); expired.limits.deadlineAt = 1;
await assert.rejects(collect(executeCapsuleOperation(context, expired)), /deadline/);
const abort = new AbortController();
const cancelled = executeCapsuleOperation(context, request(), { signal: abort.signal });
assert.equal((await cancelled.next()).value.status, 'partial');
abort.abort(new Error('cancelled'));
await assert.rejects(cancelled.next(), /cancelled/);
const early = executeCapsuleOperation(context, request());
await early.next(); const earlyCalls = calls;
await early.return(); assert.equal(calls, earlyCalls, 'closing iterator never starts next document');
closed = true;
await assert.rejects(collect(executeCapsuleOperation(context, request())), /closed/);
closed = false;
await assert.rejects(collect(executeCapsuleOperation({ ...context, program: {
  embed: async () => ({ embedding: [NaN] }) } }, request())), /finite/);
const outputLimit = request(); outputLimit.limits.maxOutputBytes = 1;
await assert.rejects(collect(executeCapsuleOperation(context, outputLimit)), /maxOutputBytes/);
let vectorIndex = 0;
await assert.rejects(collect(executeCapsuleOperation({ ...context, program: {
  embed: async () => ({ embedding: ++vectorIndex === 1 ? [1, 2] : [1] }) } }, request())), /inconsistent vector dimensions/);
await assert.rejects(collect(executeCapsuleOperation({ ...context, program: {
  embed: async () => ({ embedding: [] }) } }, request())), /empty vector/);
let embedCalls = 0;
let transitions = [];
const handle = { manifest: { modelId: 'fixture' }, supportsEmbedding: false,
  advanced: { getStats: () => ({ executionPlan: { transitions } }) },
  embed: async () => { throw new Error('Unbound embedding must not be selected.'); },
  embedWithEvidence: async () => { embedCalls++; return { embedding: [1, 2] }; } };
const programAdapter = createCapsuleProgramAdapter(handle,
  { modelId: 'fixture', program: { executionGraphHash: 'fixture' } }, { phases: { prefill: [], decode: [] } });
await assert.rejects(programAdapter.embed('one', {}), /does not declare text embedding/);
assert.equal(embedCalls, 0, 'undeclared embedding support never dispatches');
handle.supportsEmbedding = true;
const embedWithEvidence = handle.embedWithEvidence;
delete handle.embedWithEvidence;
await assert.rejects(programAdapter.embed('one', {}), /does not declare text embedding/);
assert.equal(embedCalls, 0, 'unbound embedding cannot replace the evidence contract');
handle.embedWithEvidence = embedWithEvidence;
assert.deepEqual(await programAdapter.embed('one', {}), { embedding: [1, 2] });
transitions = [{}];
await assert.rejects(programAdapter.embed('one', {}), /undeclared execution-plan transition/);
console.log('capsule-operation-document-search.test.js passed (injected program; no physical inference claim)');
