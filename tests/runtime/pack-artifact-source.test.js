...
try {
  const f = fixture();
  const shaderBytes = new TextEncoder().encode('verified Pack shader');
  ...
  shaderFixture.pack.artifacts.push(shader);
  shaderFixture.pack.wgslModules = [{ id: 'shader', file: 'example.wgsl', sourceArtifactId: shader.artifactId }];
  const shaderStore = createVerifiedPackArtifactStore(shaderFixture.pack, { readArtifact: async (entry) =>
    entry.artifactId === 'shader' ? shaderBytes : shaderFixture.store.readArtifact(entry) });
  const shaderSource = await createPackArtifactSource(shaderFixture.pack, shaderStore);
  await runWithShaderSourceScope(getStorageShaderSourceScope(shaderSource.storageContext), async () => {
    assert.equal(await loadShaderSource('example.wgsl'), 'verified Pack shader');
