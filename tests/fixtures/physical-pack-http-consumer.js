...
const pack = await read(config.packPath);
const options = await read(config.openOptionsPath);
const reference = await read(config.referencePath);
const require = createRequire(import.meta.url);
const report = { schema: 'doppler.pack-http-physical-probe/v1', passed: false,
  ...
  pack: { path: config.packPath, sha256: await digest(config.packPath) },
...
try {
  ...
  session = await openPack(config.packPath, options);
  report.loadMs = performance.now() - before;
  report.identity = session.packIdentity;
  report.targetPlanDigest = session.selectedTargetPlanDigest;
  const request = { ...config.operation, input: { ...reference.input, application: pack.release.application },
  ...
  handler = createPackServeHandler({ session, policy: config.servingPolicy, token });
