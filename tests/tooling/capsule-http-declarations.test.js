...
const filename = path.join(root, 'capsule-http-consumer.typecheck.ts');
const source = `
import http from 'node:http';
import { createCapsuleServeHandler } from './src/cli/serve/capsule-handler.js';
declare const options: Parameters<typeof createCapsuleServeHandler>[0];
const handler = createCapsuleServeHandler(options);
http.createServer(handler);
const drained: Promise<void> = handler.close();
// @ts-expect-error Explicit session authority cannot be omitted.
createCapsuleServeHandler({ policy: options.policy, token: options.token });
`;
...
console.log('capsule-http-declarations.test: ok');
